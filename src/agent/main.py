...
if __name__ == "__main__":
    ...
    parser.add_argument("--log-file", default="pipeline_logs.txt", help="Path to pipeline logs")
    args = parser.parse_args()
    
    copilot = DevOpsCopilot()
    
    # Read logs from file (passed by workflow)
    with open(args.log_file, "r", errors="ignore") as f:
        logs = f.read()
    
    if not logs.strip() or "[SYNTHETIC LOGS]" in logs:
        logging.warning("⚠️ Using limited context - results may be less precise")
    
    analysis = copilot.analyze_and_fix(logs)
    pr_url = copilot.create_fix_pr(analysis, args.log_file)
    copilot.log_metrics(analysis, pr_url)
